...
from keras.callbacks import ReduceLROnPlateau, EarlyStopping, LearningRateScheduler, CSVLogger, ModelCheckpoint, TensorBoard
...
from NetVisualizer import plot_all_feature_maps
...
def Net(net_type, w_regularize=5e-4):
    ...
    if net_type == 'EncoderDecoderResNet':
        ...
        raise Exception("Not a proper name of net type")
    return model
...
def trainNet(model,epochs, lrate, batch_size_train, train_set_numpy,train_label_set_numpy,val_set_numpy,val_label_set_numpy, net_type):
    ...
    tensorflow_callback = TensorBoard(log_dir="./logs", histogram_freq=0, write_graph=True, write_images=False)
    callback_list = [reduce_lr, csv_logger, checkpoint_writer, tensorflow_callback]
...
def trainNet(model,epochs, lrate, batch_size_train, train_set_numpy,train_label_set_numpy,val_set_numpy,val_label_set_numpy, net_type, activations_flag, weights_flag):
    ...
    callback_list = [reduce_lr, csv_logger, checkpoint_writer, tensor_board]
